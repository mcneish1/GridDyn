...
void zmqProxyHub::stopProxy ()
{
    if (proxyRunning)
    {
        ...
        controllerSocket->send (zmq::const_buffer(term.c_str(), term.size()));
        proxyThread.join ();
    }
    std::lock_guard<std::mutex> proxyLock (proxyCreationMutex);
...
void zmqProxyHub::proxyLoop ()
{
    ...
    zmq::proxy_steerable (zmq::socket_ref(zmq::from_handle, inputSocket), zmq::socket_ref(zmq::from_handle, outputSocket), zmq::socket_ref(nullptr), control);
