...
Real& DenseMatrix::operator()(int i, int j)
{
  ...
  if ( i<0 || i>=m || j<0 || j>=m )
    {
      cerr << "Bad index in DenseMatrix[" << m << "]::operator(" << i << "," << j << ")" << endl;
      abort();
    }
#endif
  if (isfacto)
...
void DenseMatrix::SetSubMat(int i, int j, const VirtualMatrix& mat, Real multcoeff)
{
  ...
  if (i<0 || i+mat_m-1>=m || j<0 || j+mat_m-1>=m)
    {
      cerr << "Bad index in DenseMatrix[" << m << "]::SetSubMat(" << i << "," << j << ",VirtualMatrix[" << mat_m << "])" << endl;
      abort();
    }
#endif
...
void DenseMatrix::AXPBY(Real alpha, Real beta, const VirtualMatrix& x)
{
  ...
  if(m!=x.GetM())
    {
      cerr << "Bad index in DenseMatrix[" << m << "]::AXPBY=(VirtualMatrix[" << x.GetM() << "])" << endl;
      abort();
    }
#endif
  if (isfacto)
...
void DenseMatrix::MatMult(Vector& b, bool transpose) const
{
  ...
  if (b.GetM()!=m)
    {
      cerr << "Bad index in DenseMatrix[" << m << "]::MatMult(Vector[" << b.GetM() << "])" << endl;
      abort();
    }
#endif
...
void DenseMatrix::Solve(Vector& b,bool transpose) const
{
  ...
  if (!isrankdef)
...
#if REAL == 1
      LAPACKE_sgetrs(LAPACK_ROW_MAJOR,char_transpose,n_lap,1,f_data,lda_lap,f_ipiv,b.GetData(),1);
#else
      LAPACKE_dgetrs(LAPACK_ROW_MAJOR,char_transpose,n_lap,1,f_data,lda_lap,f_ipiv,b.GetData(),1);
#endif
    }
  else
    {
      if (transpose)
	{
      ...
      m_lap=m;n_lap=m;lda_lap=m;ldb_lap=1;
      double rcond=-1;
      double* s=new double[m];

#if REAL == 1
      LAPACKE_sgelsd(LAPACK_ROW_MAJOR,m_lap,n_lap,1,data,lda_lap,b.GetData(),ldb_lap,s,rcond,&rank);
#else
      LAPACKE_dgelsd(LAPACK_ROW_MAJOR,m_lap,n_lap,1,data,lda_lap,b.GetData(),ldb_lap,s,rcond,&rank);
#endif
