...
SparseMatrix::SparseMatrix(int m_, int nnz_):Matrix(m_),nnz(nnz_),numeric(NULL)
{
  ...
  if (nnz>0)
    ...
    {
      col=new int[1];col[0]=0;
      data=new Real[1];data[0]=0;
    }
}
...
void SparseMatrix::Clone(const VirtualMatrix& mat)
{
  try
	...
	{
	  if (col==NULL)
	    col=new int[max(1,nnz)];
	  if (nnz>0)
	    memcpy(col,dmat.col,nnz*sizeof(int));
	}
      if (dmat.data!=NULL)
	{
	  if (data==NULL)
	    data=new Real[max(1,nnz)];
	  if (nnz>0)
	    memcpy(data,dmat.data,nnz*sizeof(Real));
	}
...
Real& SparseMatrix::operator()(int i, int j)
{
  ...
  if ( i<0 || i>=m || j<0 || j>=m )
    {
      cerr << "Bad index in SparseMatrix[" << m << "]::operator(" << i << "," << j << ")" << endl;
      abort();
    }
#endif
  if (isfacto)
...
Real& SparseMatrix::InsertVal(int i, int j, Real val)
{
  ...
  if (idx0<idx1 && col[idx0]==i)
    {
      cerr << "Try to insert a value that already exists (" << data[idx0] << ")in SparseMatrix::InsertVal(" << i << "," << j << "," << val << endl;
      abort();
    }
  else
    {
...
void SparseMatrix::SetSubMat(int i, int j, const VirtualMatrix& mat, Real multcoeff)
{
  ...
  if (i<0 || i+mat_m-1>=m || j<0 || j+mat_m-1>=m)
    {
      cerr << "Bad index in SparseMatrix[" << m << "]::SetSubMat(" << i << "," << j << ",VirtualMatrix[" << mat_m << "])" << endl;
      abort();
    }
#endif
  if (isfacto)
...
void SparseMatrix::AXPBY(Real alpha, Real beta, const VirtualMatrix& x)
{
  ...
  if(m!=x.GetM())
    {
      cerr << "Bad index in SparseMatrix[" << m << "]::AXPBY=(VirtualMatrix[" << x.GetM() << "])" << endl;
      abort();
    }
#endif
  if (isfacto)
...
void SparseMatrix::MatMult(Vector& b, bool transpose) const
{
  ...
  if (b.GetM()!=m)
    {
      cerr << "Bad index in SparseMatrix[" << m << "]::MatMult(Vector[" << b.GetM() << "])" << endl;
      abort();
    }
#endif
...
void SparseMatrix::Solve(Vector& b,bool transpose) const
{
  ...
  if (b.GetM()!=m)
    {
      cerr << "Bad index in DenseMatrix[" << m << "]::Solve(Vector[" << b.GetM() << "])" << endl;
      abort();
    }
#endif
  if (!isrankdef)
    {
      ...
      SVector x(b);
      if (transpose)
	umfpack_di_solve(UMFPACK_At,rowptr,col,data,x.GetData(),b.GetData(),numeric,NULL,NULL);
      else
	umfpack_di_solve(UMFPACK_A,rowptr,col,data,x.GetData(),b.GetData(),numeric,NULL,NULL);
      b.CopyData(x);
#endif
    }
  else
