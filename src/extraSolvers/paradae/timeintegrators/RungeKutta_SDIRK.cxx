...
Solver_App_SDIRK::Solver_App_SDIRK(Real rtol, const Vector& atol, Real tn_, Real dt_, const Vector& x0_, RungeKutta_Implicit* rk_):Solver_App_IRK(rtol,atol,tn_,dt_,x0_,rk_)
{
  current_step=0;
  b.Resize(x0_.GetM());
}
...
void Solver_App_SDIRK::EvaluateFunAndJac(const Vector& Ki, Vector& gx, bool require_jac, bool factorize)
{
  int size_x=x0.GetM();
  Real t=tn-dt;
  ...
  if (require_jac)
    {
      equation->jacobian_ypcdy(t+rk_c(current_step)*this->dt,xi,Ki,pstate,1.0/alpha,*jacmat);
