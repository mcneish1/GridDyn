...
bool RungeKutta_Implicit::SolveInnerSteps(Real t, Real used_dt, const Vector& x0, SMultiVector& allK)
{
  bool success=true;
...
Solver_App_IRK::Solver_App_IRK(Real rtol_, const Vector& atol_, Real tn_, Real dt_, const Vector& x0_, RungeKutta_Implicit* rk_):Solver_App_RK(rtol_,atol_,x0_,rk_)
{
  tn=tn_;dt=dt_;x0=x0_;
  rk_A.Clone(rk_->GetA());
  rk_c=rk_->GetC();
  equation=rk_->GetEq();
...
void Solver_App_IRK::EvaluateFunAndJac(const Vector& allK, Vector& gx, bool require_jac, bool factorize)
{
