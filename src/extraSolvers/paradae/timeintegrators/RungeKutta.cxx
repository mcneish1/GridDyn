...
{
   nb_steps_done++;
   PVector x0;val.xprev.GetPVector(0,x0);
   pstate=val.sprev;
   val.used_dt=val.next_dt;

   SMultiVector allK(nb_steps,size_x); // TODO : put allK in the calss, allocate in constr, reinit to 0 here.
   PVector Ki;
   ...
   if (success_solver)
   {
      ...
      Real refinement=1.0;
      success_error_test=this->EstimateNextStepSize(x0,val.xnext,allK,val.used_dt,refinement);
...
bool RungeKutta::EstimateNextStepSize(const Vector& x0, const Vector& x1, const SMultiVector& allK, const Real& used_dt, Real& refinement)
{
  bool success=true;
  if (do_varstep)
    {
...
Solver_App_RK::Solver_App_RK(Real rtol_, const Vector& atol_, const Vector& x0_, RungeKutta* rk_)
{
  rtol=rtol_;
