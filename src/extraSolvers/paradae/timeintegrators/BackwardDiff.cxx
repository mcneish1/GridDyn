  ...
  try
  ...
  catch (NWT_error e)
    {
      if (e == NEWTON_NOT_CONVERGED)
	    cerr << "Newton did not converge at time t=" << tn << ". Trying LineSearch keeping current solution" << endl;
      else if (e == NEWTON_INF_NAN)
	      cerr << "Newton diverged at time t=" << tn << ". Trying LineSearch discarding current solution" << endl;
      if (!do_braid)
	{
...
bool BackwardDiff::EstimateNextStepSize(const Solver_App_BDF& app, const Vector& x1, Real& refinement)
{
  bool success=true;
  if (do_varstep)
    {
...
void BackwardDiff::SetOrder(int type)
{
  ...
  switch (type)
    ...
    case 1:
      BDF_b=Real(1);
      BDF_a(0)=Real(1);BDF_a(1)=Real(-1);
      break;
    case 2:
      BDF_b=Real(2./3.);
      BDF_a(0)=Real(1);BDF_a(1)=Real(-4./3.);BDF_a(2)=Real(1./3.);
      break;
    case 3:
      BDF_b=Real(6./11.);
      BDF_a(0)=Real(1);BDF_a(1)=Real(-18./11.);BDF_a(2)=Real(9./11.);BDF_a(3)=Real(-2./11.);
      break;
    case 4:
      BDF_b=Real(12./25.);
      BDF_a(0)=Real(1);BDF_a(1)=Real(-48./25.);BDF_a(2)=Real(36./25.);BDF_a(3)=Real(-16./25.);BDF_a(4)=Real(3./25.);
      break;
    case 5:
      BDF_b=Real(60./137.);
      BDF_a(0)=Real(1);BDF_a(1)=Real(-300./137.);BDF_a(2)=Real(300./137.);BDF_a(3)=Real(-200./137.);BDF_a(4)=Real(75./137.);BDF_a(5)=Real(-12./137.);
      break;
    case 6:
      BDF_b=Real(60./147.);
      BDF_a(0)=Real(1);BDF_a(1)=Real(-360./147.);BDF_a(2)=Real(450./147.);BDF_a(3)=Real(-400./147.);BDF_a(4)=Real(225./147.);BDF_a(5)=Real(-72./147.);BDF_a(6)=Real(10./147.);
      break;
    default:
...
void BackwardDiff::ComputeBDFCoeff_FS(Real tn, const SMultiVector& tprev, const SMultiVector& xprev, const Vector& dxprev)
{
  int q=GetOrder();
...
void BackwardDiff::ComputeBDFCoeff_VS(Real tn, const SMultiVector& tprev, const SMultiVector& xprev, const Vector& dxprev)
{
  int q=GetOrder();
  SVector allh(q);Real tc=tn;
  for (int i=0;i<q;i++)
    {
      allh(i)=tc-tprev(i);
      tc=tprev(i);
  ...
  SVector maple_coef(q+1);maple_coef(1)=-allh(0)/allh(0);
  for (int i=1;i<q;i++)
  ...
  for (int i=0;i<q+1;i++)
    for (int j=0;j<q+1;j++)
      if (i!=j)
	Diag(i)/=(maple_coef(i)-maple_coef(j));
  for (int i=q;i>=0;i--)
    {
      W(i,q)=1.0;
      for (int j=q-1;j>=i;j--)
...
void BackwardDiff::ComputeBDFCoeff_FLC(Real tn, const SMultiVector& tprev, const SMultiVector& xprev, const Vector& dxprev)
{
  int q=GetOrder();
  ...
  line(0)=1;line_dot(1)=1;
  M.Solve(line);M.Solve(line_dot);
...
/*!
  ...
  \f]
*/
void BackwardDiff::ComputeBDFCoeff(Real tn, const SMultiVector& tprev, const SMultiVector& xprev, const Vector& dxprev)
{
  int q=GetOrder();
  coeff.Resize(q+2);
...
void BackwardDiff::ComputeUnknown(Vector& var, const Vector& unk, const SMultiVector& xprev, const Vector& dxprev)
{
  int q=GetOrder();
  var.CopyData(dxprev);
...
void BackwardDiff::ComputeUnknown(Vector& var, const Vector& unk, const PMultiVector& xprev, const Vector& dxprev)
{
  int q=GetOrder();
  var.CopyData(dxprev);
  var.AXPBY(coeff(0),coeff(q+1),unk);
...
Solver_App_BDF::Solver_App_BDF(Real rtol_, const Vector& atol_,Real tn_, const SMultiVector& tprev_, const SMultiVector& xprev_, const Vector& dxprev_, Vector& dxnext_, BackwardDiff* bdf_)
{
  tn=tn_;
  tprev=tprev_;
  xprev=xprev_;
...
void Solver_App_BDF::EvaluateFunAndJac(const Vector& x, Vector& gx, bool require_jac, bool factorize)
{
  ...
  if (require_jac)
	...
	{
	  equation->jacobian_ypcdy(this->tn,x,dxnext,pstate,pcoeff(0)/(tn-tprev(0)),*jacmat);
	}
