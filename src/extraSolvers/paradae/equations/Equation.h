...
public:
  ...
  RootManager():n_sroots(0),
		...
		t_sroot(0),
		tol(0){};
  RootManager(int n_sr, int n_ur, int n_st, Real tol_):n_sroots(n_sr),
						       n_uroots(n_ur),
						       n_sactive(0),
						       n_uactive(0),
						       n_state(n_st),
...
public:
  Equation();
  virtual ~Equation(){};
  virtual void function(const Real t, const Vector& y, const Vector& dy, const Vector& state, Vector& Fydy) = 0;
  virtual void jacobian_ypcdy(const Real t, const Vector& y, const Vector& dy, const Vector& state, const Real cj,Matrix& J);
  virtual void jacobian_y(const Real t, const Vector& y, const Vector& dy, const Vector& state, Matrix& Jy, bool add=false);
  virtual void jacobian_dy(const Real t, const Vector& y, const Vector& dy, const Vector& state, Matrix& Jdy,bool add=false);
  ...
  virtual void root_functions(const Real t, const Vector& y, const Vector& dy, const Vector& state, Vector& rv){};
  ...
  bool CheckUnscheduledRoots(IPoly& P, Real tlo, Vector& glo, Real& thi, Vector& ghi, Vector& iroot, Vector& state);
  // Check if a scheduled root has been crossed. If so, find the time t.
  bool CheckScheduledRoots(Real tlo, Real& thi, int& idx);

  void approx_jacobian_y(const Real t, const Vector& y, const Vector& dy, const Vector& state, Matrix& Jy, bool add=false);
  void approx_jacobian_y(const Real t, const Vector& y, const Vector& dy, const Vector& Fydy, const Vector& state, Matrix& Jy, bool add=false);
  void approx_jacobian_dy(const Real t, const Vector& y, const Vector& dy, const Vector& state, Matrix& Jy, bool add=false);
  void approx_jacobian_dy(const Real t, const Vector& y, const Vector& dy, const Vector& Fydy, const Vector& state, Matrix& Jy, bool add=false);
