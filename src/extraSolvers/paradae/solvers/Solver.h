...
public:
  ...
  virtual ~Solver_App(){};
  virtual void EvaluateFunAndJac(const Vector& x, Vector& fx, bool require_jac, bool factorize) = 0;
  ...
  virtual Real XNorm(const Vector& dx, const Vector& x, Real tol_)const{return dx.Norm2()/tol_;};
  virtual Real FxNorm(const Vector& fx, Real tol_)const{return XNorm(fx,SVector(fx.GetM(),0),tol_);};
