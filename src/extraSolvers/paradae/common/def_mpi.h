...
#define MPI_TIMER_INIT()				\
  ...
  if (rk_MACRO==0) {alldt_MACRO=new Real[sz_MACRO];}
...
#define MPI_TIMER_2(str)						\
  ...
  if (rk_MACRO==0)							\
	...
	{								\
      ...
      printf("CHECK_TIMING (%8.3f) -- %20s : %8.3f | %8.3f | %8.3f\n",MPI_Wtime()-global_time_MACRO,str,min_MACRO,avg_MACRO/sz_MACRO,max_MACRO); \
    }
#define MPI_TIMER_CLEAN() if (rk_MACRO==0) {delete[] alldt_MACRO;}

#else

#define MPI_TIMER_INIT() do {} while(0);
