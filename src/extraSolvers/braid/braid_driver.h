...
typedef struct _braid_Vector_struct
{
    ...
    griddyn::paradae::SVector state;        // JBS:  This "state" is I believe related either only to BDF or to root-finding.  Not a well named variable.

    // Vector part (this part contains actual "state" information)
    griddyn::paradae::SMultiVector xprev;   /*!< This is only a list of Vectors when multistepping, otherwise it's just one vector */
...
paradae::Real IntegrationLoop(braid_App app, std::list<paradae::Real>& tprev, std::list<paradae::Vector>& xprev, const std::list<paradae::Real>& tstops, std::list<paradae::Vector>& xstops, paradae::Vector& dxprev);
