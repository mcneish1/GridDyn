...
_braid_App_struct::_braid_App_struct(ODEProblem* ode_)
    :ode(ode_)
    , nb_multisteps(ode->GetTI()->GetType() == BDF ? ode->GetTI()->GetOrder() : 1)
    , size_x(ode->GetEq()->GetM())
    , size_state(ode->GetEq()->GetNState())
    , prevlvl(-1)
    , solution_tfinal(NULL)
...
void
my_Step_OnAllPoints(braid_App        app,
    braid_Vector     ustop,
    braid_Vector     fstop,
    braid_Vector     u,
    braid_StepStatus status,
    int level)
{
    ...
    {
        int newq;
        if (app->bdf_strat == usual || app->bdf_strat == usual_c ||
            app->bdf_strat == inject || app->bdf_strat == inject_c ||
    ...
    braid_StepStatus status)
{
    ...
    cout << "Case u=" << u << ", and ustop[" << ustop->tprev.GetM() << "]=" << ustop << ". LEVEL = " << level << endl;
    ...
    if (level == 0 || app->bdf_strat == nobdf ||
        app->bdf_strat == usual || app->bdf_strat == usual_c ||
        app->bdf_strat == inject || app->bdf_strat == inject_c ||
        app->bdf_strat == extrap || app->bdf_strat == extrap_c ||
...
int
my_SpatialRefine(braid_App              app,
    braid_Vector           cu,
    braid_Vector*          fu_ptr,
    braid_CoarsenRefStatus status)
{
    ...
    if ((app->bdf_strat == uni0 || app->bdf_strat == uni0_c ||
        app->bdf_strat == uni1 || app->bdf_strat == uni1_c ||
        app->bdf_strat == usual || app->bdf_strat == usual_c) && level != -1)
        return 0;

    Real tstart, tstop_c, tstop_f, tprev_c, tprev_f;
    braid_CoarsenRefStatusGetTpriorTstop(status, &tstart, &tprev_f, &tstop_f, &tprev_c, &tstop_c);
    ...
    */
    ...
    if (tstart == app->ode->GetEq()->GetT0())
    {
         ...
         */
            Real t;
            PVector x0; (*fu_ptr)->xprev.GetPVector(n - 1, x0);
    ...
    else
    {
        ...
        {
        case 2:
            iVan_2.Interp((*fu_ptr)->xprev, (*fu_ptr)->dxprev, new_xprev, dx, new_dt, current_dt);
            break;
        case 3:
            iVan_3.Interp((*fu_ptr)->xprev, (*fu_ptr)->dxprev, new_xprev, dx, new_dt, current_dt);
            break;
        case 4:
            iVan_4.Interp((*fu_ptr)->xprev, (*fu_ptr)->dxprev, new_xprev, dx, new_dt, current_dt);
            break;
        case 5:
            iVan_5.Interp((*fu_ptr)->xprev, (*fu_ptr)->dxprev, new_xprev, dx, new_dt, current_dt);
            break;
        case 6:
            iVan_6.Interp((*fu_ptr)->xprev, (*fu_ptr)->dxprev, new_xprev, dx, new_dt, current_dt);
            break;
        default:
...
int
my_SpatialCoarsen(braid_App              app,
    braid_Vector           fu,
    braid_Vector          *cu_ptr,
    braid_CoarsenRefStatus status)
{
    ...
    */
    ...
    if (tstart == app->ode->GetEq()->GetT0())
    {
         ...
         */
            Real t;
            PVector x0; (*cu_ptr)->xprev.GetPVector(n - 1, x0);
            DATA_Struct init_data(app->size_x, 1, app->ode->GetEq()->GetNURoots(), app->size_state);
            init_data.tprev(0) = tstart;
            init_data.xprev.CopyData(x0);
            init_data.xnext.CopyData(x0);
    ...
    else
    {
        ...
        {
        case 2:
            iVan_2.Interp((*cu_ptr)->xprev, (*cu_ptr)->dxprev, new_xprev, dx, new_dt, current_dt);
            break;
        case 3:
            iVan_3.Interp((*cu_ptr)->xprev, (*cu_ptr)->dxprev, new_xprev, dx, new_dt, current_dt);
            break;
        case 4:
            iVan_4.Interp((*cu_ptr)->xprev, (*cu_ptr)->dxprev, new_xprev, dx, new_dt, current_dt);
            break;
        case 5:
            iVan_5.Interp((*cu_ptr)->xprev, (*cu_ptr)->dxprev, new_xprev, dx, new_dt, current_dt);
            break;
        case 6:
            iVan_6.Interp((*cu_ptr)->xprev, (*cu_ptr)->dxprev, new_xprev, dx, new_dt, current_dt);
            break;
        default:
            cerr << "Should not happen" << endl;
            abort();
    ...
    braid_Vector *u_ptr)
{
    ...
    else
    {
        ...
        if (app->ode->GetEq()->GetTypeEq() == ODE)
            app->ode->GetEq()->Get_dy_from_y(u->tprev(app->nb_multisteps - 1), xi, u->state, u->dxprev);
    }
...
int
my_PropagateShell(braid_App    app,
    braid_Vector x,
    braid_Vector y)
{
    y->state = x->state;
    ...
    braid_AccessStatus astatus)
{
    ...
    else
    {
        ...
        //cout << endl;
        app->prevlvl = level;
        if (app->ode->PrintAllITERSolution())// && level == 0 && caller == braid_ASCaller_FAccess)
        {
            Real t;
            braid_AccessStatusGetT(astatus, &t);
            int iter;
    ...
    braid_BufferStatus bstatus)
{
    ...
    memcpy(dbuffer + it, u->tprev.GetData(), ns * sizeof(Real)); it += ns;
    memcpy(dbuffer + it, u->xprev.GetData(), ns*nx * sizeof(Real)); it += ns * nx;
    memcpy(dbuffer + it, u->dxprev.GetData(), nx * sizeof(Real)); it += nx;
    *(dbuffer + it) = st; it += 1;
    if (st > 0)
        memcpy(dbuffer + it, u->state.GetData(), st * sizeof(Real));
    ...
    braid_BufferStatus bstatus)
{
    ...
    memcpy(u->tprev.GetData(), dbuffer + it, ns * sizeof(Real)); it += ns;
    memcpy(u->xprev.GetData(), dbuffer + it, ns*nx * sizeof(Real)); it += ns * nx;
    memcpy(u->dxprev.GetData(), dbuffer + it, nx * sizeof(Real)); it += nx;
    st = *(dbuffer + it); it += 1;
    if (st > 0)
    {
        u->state.Resize(st);
