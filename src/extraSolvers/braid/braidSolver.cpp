...
void BuildGrid(ODEProblem* ode, const MapParam& param, int& Nsteps, Real*& timegrid)
{
    ...
    else
    {
        ...
        if (!ode->GetTI()->DoBraid())
            ode->GetTI()->DoFalseVarstep() = true;
...
int braidSolver::RunBraid(ODEProblem* ode, MapParam* param, Real* &timegrid, int Ngridpoints)
{
    ...
    if (TI->GetType() == BDF)
    {
        ...
        if (min_order < 1)
            min_order = 1;
        if (min_order > app->nb_multisteps)
            min_order = app->nb_multisteps;
    ...
    braid_Core core;
    braid_Init(MPI_COMM_WORLD, ode->GetComm(), t0, Tmax, braid_Nsteps, app,
