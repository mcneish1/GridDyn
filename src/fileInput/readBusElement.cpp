...
gridBus *readBusElement (std::shared_ptr<readerElement> &element, readerInfo &ri, coreObject *searchObject)
{
    ...
    gridBus *bus = ElementReaderSetup (element, static_cast<gridBus *>(nullptr), busComponentName, ri, searchObject);
        ...
        else
        {
            ...
            // how it interprets the state
            {
              ...
              const std::invalid_argument
                &)  // either invalidParameterValue or unrecognizedParameter depending on the actual model used
            {
