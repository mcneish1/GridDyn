...
void OperatingBoundary::updateRange (double val) const
{
    if (val > cRangeHigh)
    {
        auto vLoc = std::lower_bound (operatingPoints.begin () + lastIndex, operatingPoints.end (),
                                      opPoint{val, 0, 0}, compareOp);
        lastIndex = vLoc - operatingPoints.begin ();
    }
    else
    {
        auto vLoc = std::lower_bound (operatingPoints.begin (), operatingPoints.begin () + lastIndex,
                                      opPoint{val, 0, 0}, compareOp);
        lastIndex = vLoc - operatingPoints.begin ();
    }
    if (lastIndex == 0)
