...
void gridDynServer::send_data()
{
	...
	while(1)
	{
		...
		if (ip_protocol==udp)
		{
			...
			udpsock->socket_.async_send_to(boost::asio::buffer(dataFrame),remote_endpoint_udp_send,
				boost::bind(&pmu_udp_socket::data_sent,udpsock,
		...
		else if (ip_protocol==tcp)
		{
			...
			for (kk=0;kk<active_tcp_sessions.size();kk++)
			{
				...
				if (active_tcp_sessions[kk]->send_lock.try_lock())
				{
					...
					active_tcp_sessions[kk]->socket_.async_send(boost::asio::buffer(dataFrame),
						boost::bind(&pmu_tcp_session::data_sent,active_tcp_sessions[kk],
						boost::asio::placeholders::error,
						boost::asio::placeholders::bytes_transferred));
				}
				else
			...
			if (skipped_some)
			{
				for (kk=0;kk<skipped.size();kk++)
				{
					if (active_tcp_sessions[skipped[kk]]->send_lock.try_lock())
					{
						...
						active_tcp_sessions[skipped[kk]]->socket_.async_send(boost::asio::buffer(dataFrame),
...
void gridDynServer::tcp_accept(pmu_tcp_session *active_session,const boost::system::error_code &error)
{
	...
	if (!error)
	{
        ...
        boost::bind(&gridDynServer::pmu_tcp, this,active_session,
          boost::asio::placeholders::error,
          boost::asio::placeholders::bytes_transferred));
...
void gridDynServer::pmu_tcp(pmu_tcp_session *active_session,const boost::system::error_code &error, std::size_t size)
{
	unsigned char c;
	ptime currTime;
	int id_pdc=0;
