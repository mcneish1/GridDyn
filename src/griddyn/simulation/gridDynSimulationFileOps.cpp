...
void savePowerFlowTXT (gridDynSimulation *gds, const std::string &fileName)
{
    ...
    while (Area != nullptr)
    {
        ...
        while (bus != nullptr)
        {
            fprintf (fp, "%d\t\t%d\t\t\"%-20s\"\t %5d\t%5d\t %7.5f\t %7.5f\t %7.5f\t %7.5f\t %7.5f\n",
                     Area->getUserID (), lnk->getUserID (), lnk->getName ().c_str (),
                     lnk->getBus (1)->getUserID (), lnk->getBus (2)->getUserID (),
    ...
    {
        fprintf (fp, "%d\t\t %d\t\t\"%-20s\"\t %5d, %5d\t %7.5f\t %7.5f\t %7.5f\t %7.5f\t %7.5f\n", 1,
                 lnk->getUserID (), lnk->getName ().c_str (), lnk->getBus (1)->getUserID (),
                 lnk->getBus (2)->getUserID (), lnk->getRealPower (1) * basePower,
                 lnk->getReactivePower (1) * basePower, lnk->getRealPower (2) * basePower,
                 lnk->getReactivePower (2) * basePower, lnk->getLoss () * basePower);
    ...
    while (Area != nullptr)
    {
        fprintf (fp, "%d\t\t\"%-20s\"\t %7.2f\t %7.2f\t %7.2f\t %7.2f\t %7.2f\t %7.2f\n", Area->getUserID (),
                 Area->getName ().c_str (), Area->getGenerationReal () * basePower,
    ...
    fprintf (fp, "%d\t\t\"%-20s\"\t %7.2f\t %7.2f\t %7.2f\t %7.2f\t %7.2f\t %7.2f\n", 1, gds->getName ().c_str (),
             gds->getGenerationReal () * basePower, gds->getGenerationReactive () * basePower,
             gds->getLoadReal () * basePower, gds->getLoadReactive () * basePower, gds->getLoss () * basePower,
             -99999.0);
...
void cdfBusPrint (FILE *fp, int areaNum, gridBus *bus)
{
    ...
    if ((type == 1) || (type == 3))
    {
        fprintf (fp, " %7.1f %6.3f%8.2f%8.2f", bus->get ("basevoltage"), bus->get ("vtarget"), vmax, vmin);
    }
    else
    {
...
void cdfLinkPrint (FILE *fp, int areaNum, acLine *lnk)
{
    ...
    fprintf (fp, "%5d %5d %5d %4d %1d  ", static_cast<int> (rat1), static_cast<int> (rat2),
             static_cast<int> (rat3), cbus, 0);
    switch (type)
    {
    case 0:
    ...
    case 2:
    case 3:
        fprintf (fp, "%6.4f %6.1f %6.4f %6.4f%7.5f %6.4f %6.4f\n", lnk->getTap (), 0.0, minAdj, maxAdj, ssize,
                 minVal, maxVal);
        break;
    case 4:
        fprintf (fp, "%6.4f%7.2f%7.4f%7.2f %6.2f%7.1f%7.1f\n", lnk->getTap (), lnk->getTapAngle () * 180.0 / kPI,
                 minAdj, maxAdj, ssize, minVal, maxVal);
        break;
    }
}
...
void saveStateXML (gridDynSimulation * /*gds*/, const std::string & /*fileName*/, const solverMode & /*sMode*/) {}

void saveStateBinary (gridDynSimulation *gds, const std::string &fileName, const solverMode &sMode, bool append)
{
...
void loadStateXML (gridDynSimulation * /*gds*/, const std::string & /*fileName*/, const solverMode & /*sMode*/) {}
