...
void gridSimulation::add (std::shared_ptr<Event> evnt) { EvQ->insert (std::move (evnt)); }

void gridSimulation::add (std::shared_ptr<eventAdapter> eA) { EvQ->insert (std::move (eA)); }
...
void gridSimulation::getEventObjects (std::vector<coreObject *> &objV) const { EvQ->getEventObjects (objV); }

int gridSimulation::run (coreTime /*finishTime*/) { return FUNCTION_EXECUTION_FAILURE; }
...
void gridSimulation::log (coreObject *object, print_level level, const std::string &message)
{
    ...
    if (!(customLogger))
    {
        ...
        {
            gridLog->startLogging (static_cast<int> (consolePrintLevel), static_cast<int> (logPrintLevel));
        }
    }
    ...
    std::string cname = '[' + (isSameObject (object, this) ?
                        ...
                        ']';
    std::string simtime =
      ((currentTime > negTime) ? '(' + std::to_string (currentTime) + ')' : std::string ("(PRESTART)"));
    std::string key;
...
coreTime gridSimulation::getEventTime () const { return EvQ->getNextTime (); }

coreTime gridSimulation::getEventTime (int eventCode) const { return EvQ->getNextTime (eventCode); }
