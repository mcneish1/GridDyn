...
void gridDynSimulation::setInstance (gridDynSimulation *gds) { s_instance = gds; }

gridDynSimulation *gridDynSimulation::getInstance () { return s_instance; }
    ...
    for (int32_t nn = 1; nn <= networkNum; nn++)
    {
        ...
        if (!slkfnd)
        {
            if (controlFlags[no_auto_slack_bus])
            {
                ...
                LOG_WARNING ("no SLK bus found in network " + std::to_string (nn) + " disconnecting buses");
                for (auto &bn : bnetwork)
                {
                    if (bn->Network == nn)
                    {
...
double gridDynSimulation::getState (index_t offset) const { return getState (offset, cLocalSolverMode); }
...
void gridDynSimulation::add (const std::string &actionString) { actionQueue.emplace (actionString); }

void gridDynSimulation::add (gridDynAction &newAction) { actionQueue.push (newAction); }
...
solverMode gridDynSimulation::getSolverMode (const std::string &solverType)
{
    ...
    if ((solverType == "algebraic_only") || (solverType == "algebraic") || (solverType == "dynalg"))
    {
        return *defDynAlgMode;
    }
    if ((solverType == "differential_only") || (solverType == "differential") || (solverType == "dyndiff"))
    {
...
std::shared_ptr<SolverInterface> gridDynSimulation::updateSolver (const solverMode &sMode)
{
    ...
    else if (pState >= gridState_t::DYNAMIC_INITIALIZED)
    {
        ...
        guessState (currentTime, sd->state_data (), (hasDifferential (sMode)) ? sd->deriv_data () : nullptr, sm);
