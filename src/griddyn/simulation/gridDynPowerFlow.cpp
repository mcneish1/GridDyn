...
#include "griddyn/simulation/continuation.h"
    ...
    if (pFlowData->size () > 0)  // handle the condition when all buses are swing buses hence nothing to solve
    {
        ...
        do  // outer power distribution loop
        {
            ...
            do
            {
                ...
                if (pfer.attempts () > 0)
                {
                    if (!std::all_of (pFlowData->state_data (), pFlowData->state_data () + pFlowData->size (),
                ...
                if (!controlFlags[no_powerflow_adjustments])
                {
                    ...
                    if (pState == gridState_t::INITIALIZED)
                    {
                        ...
                        {
                            AdjustmentChanges = powerFlowAdjust (noInputs, lower_flags (controlFlags),
                    ...
                    {
                        AdjustmentChanges =
                          powerFlowAdjust (noInputs, lower_flags (controlFlags), check_level_t::reversable_only);
                    }
                    ...
                    if (AdjustmentChanges == change_code::no_change)  // if there was no adjustable changes  check
                    // if there was any non-reversable checks
                    {
                        AdjustmentChanges =
                          powerFlowAdjust (noInputs, lower_flags (controlFlags), check_level_t::full_check);
