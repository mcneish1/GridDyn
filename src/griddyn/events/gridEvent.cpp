...
Event::Event(gridEventInfo &gdEI, coreObject *rootObject)
{
	eventId = static_cast<count_t>(getID());
	Event::updateEvent(gdEI, rootObject);
}

void Event::updateEvent(gridEventInfo &gdEI, coreObject *rootObject)
{
...
std::unique_ptr<Event> make_event (gridEventInfo &gdEI, coreObject *rootObject)
{
