    ...
    if (isDynamic (sMode))
    {
        ...
        else
        {
            ...
            md.assignCheckCol (QoutLocation, B2Voffset,
                               -1 * (-Idc * linkInfo.v2 / std::sqrt (k3sq2 * k3sq2 * linkInfo.v1 * linkInfo.v1 -
        ...
        else
        {
            if (B2Voffset != kNullLocation)
            {
                ...
                else
                {
                    ...
                    md.assignCheckCol (QoutLocation, B2Voffset, linkInfo.v1 / tap * linkInfo.v2 / temp);
                }
            }
        }
    }
...
void acdcConverter::residual (const IOdata &inputs, const stateData &sD, double resid[], const solverMode &sMode)
{
    ...
    if (isDynamic (sMode))
    {
        ...
        {
            double Padj =
              (tD > 0.0001) ? controlDelay->getOutput (a, sD, sMode) : powerLevelControl->getOutput (a, sD, sMode);
            tap = baseTap / (1.0 + dirMult * baseTap * Padj);
        }
        double I0 = linkInfo.v1 / tap;
...
void acdcConverter::guessState (coreTime time, double state[], double dstate_dt[], const solverMode &sMode)
{
    if (isDynamic (sMode))
    {
...
void acdcConverter::updateLocalCache (const IOdata & /*inputs*/, const stateData &sD, const solverMode &sMode)
{
