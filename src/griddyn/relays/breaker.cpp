...
void breaker::setState (coreTime time, const double state[], const double /*dstate_dt*/[], const solverMode &sMode)
{
    if (useCTI)
    {
        auto offset = offsets.getDiffOffset (sMode);
...
void breaker::guessState (const coreTime /*time*/, double state[], double dstate_dt[], const solverMode &sMode)
{
    if (useCTI)
    {
        auto offset = offsets.getDiffOffset (sMode);
