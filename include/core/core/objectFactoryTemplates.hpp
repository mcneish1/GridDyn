...
class gridObjectHolder : public coreObject
{
  ...
  private:
    std::vector<Ntype> objArray;  // vector of objects, this should not resize or change once created
    count_t next = 0;
    count_t objCount = 0;
  ...
  public:
    explicit gridObjectHolder (count_t objs) : coreObject ("holder_#"), objArray (objs), objCount (objs)
    {
        ...
        {  // we want to add an owning reference since these objects are held internally and should not be deleted
            // elsewhere
...
class typeFactory : public objectFactory
{
  ...
  public:
    ...
    typeFactory (const std::string &component, const stringVec &typeNames) : objectFactory (component, typeNames)
    {
        auto tF = coreObjectFactory::instance ()->getFactory (component);
    ...
    typeFactory (const std::string &component, const stringVec &typeNames, const std::string &defType)
...
class childTypeFactory : public typeFactory<Btype>
{
  ...
  public:
    ...
    childTypeFactory (const std::string &component, const stringVec &typeNames, const std::string &defType)
