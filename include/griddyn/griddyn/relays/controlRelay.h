...
class controlRelay : public Relay
{
  ...
  protected:
    ...
    generateGetEvent (coreTime eventTime, std::uint64_t sourceID, comms::controlMessagePayload *message);
    std::unique_ptr<functionEventAdapter>
    generateSetEvent (coreTime eventTime, std::uint64_t sourceID, comms::controlMessagePayload *message);
};
}  // namespace relays
}  // namespace griddyn
