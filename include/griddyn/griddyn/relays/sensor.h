...
class sensor : public Relay
{
  public:
    ...
    enum sensor_flags
    {
        direct_IO = object_flag6,  //!< indication that the sensor is directly listing all inputs as outputs with
                                   //!< no processing
        ...
        force_continuous = object_flag10,  //!< force continuous operation even if the underlying data sources are
                                           //!< not continously available
    };
  ...
  private:
    ...
    */
    ...
    double
    getBlockInput (index_t blockNum, const IOdata &inputs, const stateData &sD, const solverMode &sMode) const;
