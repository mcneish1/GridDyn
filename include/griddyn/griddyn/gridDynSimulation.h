...
class gridDynSimulation : public gridSimulation
{
  ...
  protected:
    ...
    dynamic_solver_methods defaultDynamicSolverMethod =
      dynamic_solver_methods::dae;  //!< specifies which dynamic solver method to use if it is not otherwise
                                    //! specified.
  ...
  public:
    ...
    */
    int checkNetwork (network_check_type checkType);  // function to do a check on the network and potentially
  ...
  @return in indicating success (0) or failure (non-zero)*/
    ...
    */
    ...
    @param[in] time  the simulation time of the evaluation
    @param[in] indices the indices of the paramters
...
*/
  ...
  */
    ...
    */
    ...
    @param[in] val the value to the set the property to
    */
    virtual void solverSet (const std::string &solverName, const std::string &field, const std::string &val);
