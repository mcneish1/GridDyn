...
protected:
  ...
  std::vector<Link *> terminalLink;  //!< list of internal links which make the external connections
...
public:
  ...
  */
  virtual void converge (coreTime time, double state[], double dstate_dt[], const solverMode &sMode, converge_mode mode = converge_mode::block_iteration, double tol = 0.01) override;
  ...
  */
  ...
  virtual double getRealPower (id_type_t busId = invalid_id_value) const override;      //function to return the real flow in
  virtual double getReactivePower (id_type_t busId = invalid_id_value) const override;      //function to return the reactive power in
  ...
  virtual IOdata getOutputs (id_type_t busId, const stateData &sD, const solverMode &sMode) const override;
