...
#include "fmi/fmi_import/fmiMESubModel.h"
#include "fmi/fmi_import/fmiLibraryManager.h"
...
void fmiMESubModel::derivative (const IOdata &inputs, const stateData &sD, double deriv[], const solverMode &sMode)
{
    ...
    if (isDynamic (sMode))
    {
        ...
        printf ("tt=%f,I=%f, state=%f deriv=%e\n", static_cast<double> (sD.time), inputs[0], Loc.diffStateLoc[0],
                Loc.destDiffLoc[0]);
    }
    else
    {
        me->getDerivatives (Loc.destLoc);
        printf ("tt=%f,I=%f, state=%f,deriv=%e\n", static_cast<double> (sD.time), inputs[0], Loc.algStateLoc[0],
                Loc.destLoc[0]);
    }
}
    ...
    if (me->getCurrentMode () >= fmuMode::initializationMode)
    {
            ...
            (isDynamic (sMode)))
        {
            for (index_t pp = 0; pp < m_outputSize; ++pp)
            {
                ...
                {
                    const double res =
                      oEst[pp]->estimate (sD.time, inputs, sD.state + offsets.getDiffOffset (sMode));
                    out[pp] = res;
                }
            }
        }
